...
\textit{Spectral roll-off} & $ \sum_{0}^{f_c}a^2(f) = 0.95 \sum_{0}^{f_s / 2}a^2(f)$                                                   \\ \hline
\textit{Spectral flux}     & $X_{\mathrm{flux}} = 1 - \frac{\sum_k a(t-1, k) \cdot a(t,k)}{\sqrt{\sum_k a(t-1, k)^2} \sqrt{\sum_k a(t, k)^2}}$ \\ \hline 
...
\textit{Energy}            & $ E = \sum_{i = 0}^{N - 1}|x(t)|^2 $                                                                    \\ \hline \textit{Energy ratio}                & $E_i = \frac{E_i}{\sum_{i = 1}^{N}E_i} $                                                        \\ \hline
...
\myequations{Empirical scaling function}
\begin{ceqn}\begin{align}
\hat{\phi_n}(\omega) = 
\begin{cases}
...
\myequations{Empirical wavelet function}
\begin{ceqn}\begin{align}
\hat{\psi_n}(\omega) = 
\begin{cases}
...
Refinements acting in the onset as preparation before wavelet processing create an envelope curve in the frequency domain and attempts to eliminate supposed interference. 
