...
In a year the system gathers 31.54 Gs (gigasamples) which is 58.74 GiB with 16-bit ADC resolution. Reasonably precise spectral estimation with 10 thousand bins needs 3.15 Gs per year. On the other hand, 6 features out of each channel keep only 1.89 Ms per year for a lossy compression ratio of $6 \times 10^{-5}$. Low data volumes potentially enable feature selection and models to be offloaded directly to edge devices. The entire machine's history can be kept in a small flash memory module.
...
We utilize the k-nearest neighbor algorithm to check machinery diagnostics abilities with reduced feature sets. kNN classifier being a lazy learner means it can be adapted easily from offline to online context. Training labels establish nearest-neighbor decision boundaries for attribute values. Voronoi diagrams can display these regions and explain the model in that way. Batch learning in kNN serves here as a target performance whose attainment is desirable with online learning.

\subsection{Batch models}
Three types of kNN model experiments run with hold-out validation. 
...
Two models are created for the target variable in classification with all 10 temporal and 11 spectral features. The subset of records includes all rotational speeds on bearing position A. Either six fault labels or anomaly above 0.9 severity level are guessed by the model with the same hold-out validation split as before. The number of k neighbors is 5, the distance metric is the Euclidian norm, and the algorithm for proximity queries is a k-d tree. 

Confusion matrices for fault predicion and high severity anomaly are shown in Figure \ref{fig:design:knn-confusion-matrix}. The most inaccuracies in the temporal domain are between misalignment and bearing race faults. In the spectral domain, the model confuses imbalance and bearing faults. It is to be expected since mass to unbalance the shaft is hanged to simulate bearing defects. 
...
\multirow{2}{*}{A}                                                                   & Fault                                                                               & \begin{tabular}[c]{@{}l@{}}$\sum 2998$\\ 498 pre class\end{tabular}  & \multicolumn{1}{r|}{0.9413}                                                               & \multicolumn{1}{r|}{0.9093}                                                              & 0.91                                                                                      & \multicolumn{1}{r|}{0.9917}                                                               & \multicolumn{1}{r|}{0.9836}                                                              & 0.98                                                                                      \\ \cline{2-9} 
...
Lowering the number of attributes allows for visualization of labels in planar scatter plots. The data points in 3-dimensional cross sections are colored by true fault classes of all severities in Figure \ref{fig:design:feature-space-scatter}. We observe the inability to separate faults by linear boundaries in given feature spaces and the noncompactness of clusters. 

Overlapping clusters are evidenced by relatively low silhouette scores around zero. Therefore, we cannot increase the k hyperparameter as much because otherwise, categories merge. Relabeling according to severity should be undertaken. Low severity faults should be reassigned to a class with no fault.
...
\begin{figure}[ht]
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \caption{Spectral domain features}
    \end{subfigure} 
    \caption{Cross sections of 3-dimensional feature spaces for best attributes chosen by rank product method}
...
%TODO
% Model performance on feature selection techniques is compared on 3 features to set of all features and best ever combinations of triplets. Three principal components computed out of all features in each domain are also used in comparison. (significant overlap) We suspect the PCA property of maximizing variances in each dimention support separating groups in classification. 

% Results of experiments in the scenarios is aggregated in Figure X. Best combinations of 3 features is always better than all 10 features because of overfitting. PCA is always better than any other method of feature selection. Rank product is not universally  better but balances other methods to achieve more stable results. 

% In majority of situations it it better than all three individually. Subset of spectral features have better performance than temporal features, presumbly because of many correlated pairs of features. In would be interesting to investigate how many features is optimal by giving yet better performance and altering feature election process.
